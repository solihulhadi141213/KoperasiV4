<?php
    ...
    // CEK DATA
    $Qry = $Conn->prepare("SELECT id_pinjaman_jenis FROM pinjaman_jenis WHERE id_pinjaman_jenis = ? LIMIT 1");
    if (!$Qry) {
        echo json_encode([
            "status"  => "error",
    ...
    // HAPUS DATA
    $Delete = $Conn->prepare("DELETE FROM pinjaman_jenis WHERE id_pinjaman_jenis = ? LIMIT 1");
    if (!$Delete) {
        echo json_encode([
            "status"  => "error",
